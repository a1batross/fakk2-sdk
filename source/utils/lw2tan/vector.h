...
public:
	...
	Quaternion( float _r, float _x, float _y, float _z )
	{
		r = _r;
		x = _x;
		y = _y;
	...
	float angle()
	{
		return (float)( acos( r ) * 2.0 );
	}

	Vector axis()
	{
		Vector a( x, y, z );
		return a * (float)( 1 / sin( angle() / 2.0 ));
	}
