...
int SkelExport::GetNodeIndex( INode *node )
{
	int i;
...
void SkelExport::GetNonRigidVertex( INode *node, loadvertx_t *vert, int index )
{
	Point3  p;
	Matrix3 tm;
...
void SkelExport::GetPhysiqueVertex( loadvertx_t *vert, int index )
{
...
Modifier *SkelExport::FindPhysiqueModifier( INode *nodePtr )
{
	int            ModStackIndex;
...
void SkelExport::GetVertexNormal( int vertindex, int smoothingGroup, float normal[ 3 ] )
{
	int     numnormals;
	int     i;
	int     num;
...
void SkelExport::AddObject( Object *obj, INode *node )
{
	TriObject       *tri;
...
void SkelExport::AddBoneFrame( Object *obj, INode *node )
{
	int     bonenum;
	Matrix3 orientation;
	Point3  origin;
...
void SkelExport::CollectMaterials( void )
{
	int i;
	int j;
...
void SkelExport::SortObjectNames( Container<int> &sorted )
{
	int i;
	int j;
	int num;
...
void SkelExport::SaveAnimFile( const char *filename, loadframe_t *anim, int numframes, float framerate )
{
	FILE *out;
	int  i;
	int  j;
...
void SkelExport::SaveAnimFileText( const char *filename, loadframe_t *anim, int numframes, float framerate )
{
	FILE           *out;
	int            i;
	int            j;
...
BOOL CALLBACK SkelExport::StatusDlgProc( HWND   hwndDlg, // handle to dialog box
					 UINT   uMsg,    // message
...
BOOL CALLBACK SkelExport::GetAnimScaleDlgProc( HWND   hwndDlg, // handle to dialog box
					       UINT   uMsg,    // message
					       WPARAM wParam,  // first message parameter
					       LPARAM lParam   // second message parameter
					       )
...
void SkelExport::GetAnimScale( void )
{
...
void SkelExport::ShowProgressBar( const char *title )
{
...
void SkelExport::UpdateProgressBar( float percentComplete )
{
	SendMessage( progressBar, PBM_SETPOS, int(percentComplete * 100), 0 );
}

void SkelExport::SetStatusText( const char *text )
{
	SetWindowText( statusText, text );
}

void SkelExport::CloseProgressBar( void )
{
	if( statusDialog )
	{
...
const TCHAR *SkelExport::Ext( int n )
{
	switch( n )
	{
	case 0:
...
const TCHAR *SkelExport::LongDesc( void )
{
...
const TCHAR *SkelExport::ShortDesc( void )
{
...
const TCHAR *SkelExport::AuthorName( void )
{
	return _T( AUTHOR );
}
...
const TCHAR *SkelExport::CopyrightMessage( void )
{
	return _T( COPYRIGHT );
}
...
const TCHAR *SkelExport::OtherMessage1( void )
{
...
const TCHAR *SkelExport::OtherMessage2( void )
{
...
unsigned int SkelExport::Version( void )
{
	return VERSION;
}
...
void SkelExport::ShowAbout( HWND hWnd )
{
}
