...
bool DrawingObject::Properties( void )
{
	CObjProps props;
...
DrawingObject::hits DrawingObject::HitTest( CPoint pt )
{
	if( !rect.PtInRect( pt ))
		return DO_HIT_NONE;
...
void DrawingObject::Draw( CDC &dc )
{
#if 0
	if( active )
	{
...
DrawingObjectList::DrawingObjectList( void ) : workspaceSize( 640, 480 ), workspaceName( "UnamedMenu" ), workspaceMotion( "NONE" ), time( "1" )
{
}

bool DrawingObjectList::SaveAs( void )
{
	CString     newName;
...
bool DrawingObjectList::Save( void )
{
	if( m_fileName == "" )
	{
		return SaveAs();
...
bool DrawingObjectList::Open( void )
{
	int         i;
	CString     newName;
...
void DrawingObjectList::operator =( const DrawingObjectList &other )
{
	POSITION pos;
