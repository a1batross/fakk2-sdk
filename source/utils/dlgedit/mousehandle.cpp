...
DefaultMouseEventHandler::DefaultMouseEventHandler( void )
{
	SetCapture( NULL );
}

void DefaultMouseEventHandler::RightClick( CPoint pt )
{
	POSITION pos;
	lastClicked = NULL;
...
void DefaultMouseEventHandler::LeftDoubleClick( CPoint pt )
{
	POSITION pos;
...
void DefaultMouseEventHandler::LeftButtonDown( int keys, CPoint pt )
{
	POSITION pos;
	bool     cleared = false;
...
MoveXYHandler::MoveXYHandler( long *x, long *y, CPoint curMousePos, CPoint _mins, CPoint _maxes ) : px( x ), py( y ), orig( *x, *y ), mins( _mins ), maxes( _maxes )
{
...
void MoveXYHandler::MouseMove( int keys, CPoint pt )
{
...
void MoveXYHandler::LeftButtonUp( int keys, CPoint pt )
{
...
void MoveXYHandler::Cancel( void )
{
	*px = orig.x;
	*py = orig.y;
...
MoveRectHandler::MoveRectHandler( CRect *rect, CPoint curMousePos, CPoint _mins, CPoint _maxes ) :
...
void MoveRectHandler::MouseMove( int keys, CPoint pt )
{
	topleft.MouseMove( keys, pt );
	botright.MouseMove( keys, pt );
}

void MoveRectHandler::LeftButtonUp( int keys, CPoint pt )
{
...
void MoveRectHandler::Cancel( void )
{
	topleft.Cancel();
	botright.Cancel();
