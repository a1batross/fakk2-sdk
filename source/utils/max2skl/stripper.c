...
void OrderMesh( int input[][3], int output[][3], int numTris )
{
	...
	while( sumStrippedTriangles < numTris )
	{
		...
		totalStrips++;
		qprintf( " strips %4d stripped %5d/%5d last strip length %4d\r", totalStrips, sumStrippedTriangles, numTris, strippedTriangles );
	}

	qprintf( "Triangles on surface:		%d\n", sumStrippedTriangles );
	qprintf( "Total strips from surface: %d\n", totalStrips );
	qprintf( "Average strip length:      %.1f\n", (float) sumStrippedTriangles / totalStrips );
}
