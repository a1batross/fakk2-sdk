...
Tri::Tri( Vert *v0, Vert *v1, Vert *v2 )
{
	int i;
	int j;
...
int Tri::HasVertex( Vert *v )
{
	return( v == vertex[ 0 ] || v == vertex[ 1 ] || v == vertex[ 2 ] );
}

void Tri::ComputeNormal( void )
{
	Vector v0;
	Vector v1;
	Vector v2;
...
Tri *Tri::GetAdjacentTriangle( int edge )
{
	int  i;
	Vert *u;
	Vert *v;
...
Vector ProjectPointOnPlane( const Vector p, const Vector normal )
{
	float  d;
	Vector n;
	float  inv_denom;
...
////////////////////////
Vector PerpendicularVector( const Vector src )
{
	int    pos;
	int    i;
	float  minelem;
...
Vector RotatePointAroundVector( const Vector dir, const Vector point, float degrees )
{
	matrix im;
	matrix zrot;
	matrix tmpmat;
...
void Vert::RemoveIfNonNeighbor( Vert *n )
{
	int i;
...
void SkelModel::FixMirroredBones( void )
{
	int             i;
	int             j;
	loadframe_t     *frame;
