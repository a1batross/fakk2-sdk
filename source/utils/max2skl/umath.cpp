...
int umath::RandomInt( void )
{
	s_seed = 69069 * s_seed + 1;
	return s_seed;
}

float umath::RandomFloat( void )
{
	return ( RandomInt() & 0xFFFF ) / float(0x10000);
}

float umath::RandomSignedFloat( void )
{
	return 2.f * ( RandomFloat() - 0.5f );
}

#if 1
inline float umath::InvSqrt( float number )
{
	float y;
...
__declspec( naked ) float umath::InvSqrt( float f )
{
	__asm
	{
		fld dword ptr [esp + 4]
