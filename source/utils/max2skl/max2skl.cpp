...
void SkelModel::LoadIgnoreFile( const char *filename )
{
	Script script;
...
bool SkelModel::IgnoreSurface( const char *name )
{
	int i;
...
void SkelModel::LoadSKL( const char *filename )
{
	Script script;
	int    version;
...
void SkelModel::LoadSurfaces( Script &script )
{
	int       i;
	surface_t *surf;
	int       num;
...
void SkelModel::LoadBones( Script &script )
{
	skelBoneName_t *bone;
	int i;
	int bonenum;
...
void SkelModel::LoadVerts( Script &script )
{
	blendvert_t *blend;
	loadvertx_t *vert;
	int         j;
...
void SkelModel::LoadFaces( Script &script )
{
...
void SkipFrame( Script &script, int numbones )
{
	int j;
...
void SkelModel::LoadFrames( Script &script )
{
	loadbone_t *bone;
	int        k;
	int        j;
...
void SkelModel::CopyBaseModel( SkelModel &base )
{
	int i;
...
void SkelModel::CopyBones( SkelModel &base )
{
	loadvertx_t *vert;
	loadframe_t *frame;
...
void SkelModel::LoadUVFile( const char *filename )
{
	int       i;
	int       j;
	int       version;
...
void SkelModel::CreateSurfaces( void )
{
	int            i;
	int            j;
	int            k;
...
void SkelModel::CalcFrameBones( int framenum, UMat3 bones[ MAX_BONES ], UVector3 offsets[ MAX_BONES ] )
{
	int         i;
	int         parent;
	loadframe_t *frame;
...
void SkelModel::CalcVerts( vec3_t *outverts, UMat3 bones[ MAX_BONES ], UVector3 offsets[ MAX_BONES ] )
{
	int         i;
	int         j;
	loadvertx_t *vert;
...
void SkelModel::CalcFrame( int framenum, vec3_t *outverts )
{
...
void SkelModel::ComputeVertexNormals( void )
{
	UMat3     bones[ MAX_BONES ];
	UVector3  offsets[ MAX_BONES ];
	surface_t *surf;
...
void SkelModel::CalculateLOD( void )
{
	int         i;
	int         j;
	int         minresolution;
...
void SkelModel::CalculateTriStrips( void )
{
	int       s;
	surface_t *surf;
...
void SkelModel::CalculateBounds( void )
{
	int         i;
	int         j;
	int         k;
...
void SkelModel::ComputeTagFromTri( loadbone_t *bone, const float pTri[ 3 ][ 3 ] )
{
	float  len[ 3 ];
	vec3_t axis[ 3 ];
	vec3_t sides[ 3 ];
...
void SkelModel::CalculateTags( void )
{
	loadframe_t *frame;
	surface_t   *surf;
	float       tri[ 3 ][ 3 ];
...
void SkelModel::ScaleModel( float scale )
{
	loadframe_t *frame;
	loadvertx_t *vert;
	loadbone_t  *bone;
...
void SkelModel::ReverseAnimation( void )
{
	loadframe_t *framelo;
	loadframe_t *framehi;
	loadbone_t  *bone;
...
void SkelModel::ReduceBonesPerVertex( int maxbones, float minweight )
{
	loadvertx_t *vert;
	loadvertx_t v;
	int         i;
...
int SkelModel::AddBone( int parent, const char *name )
{
...
void SkelModel::RemoveUnusedBones( void )
{
	surface_t      *surf;
	loadvertx_t    *vert;
	int            i;
...
void SkelModel::ConvertToQuake3Worldspace( void )
{
	int         i;
	int         j;
	loadframe_t *frame;
...
void SkelModel::SaveBaseFrame( const char *name )
{
	int            i;
	int            j;
	int            k;
...
void SkelModel::SaveAnimation( const char *name )
{
	int              i, j, k;
	FILE             *modelouthandle;
	loadframe_t      *frame;
...
void SkelModel::SaveMD4( const char *name )
{
	int           i;
	int           j;
	int           k;
