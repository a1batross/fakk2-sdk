...
UStringStreamIn &UStringStreamIn::operator <<( int i )
{
...
UStringStreamIn &UStringStreamIn::operator <<( unsigned u )
{
	if( m_intleadwithspaces )
...
UStringStreamIn &UStringStreamIn::operator <<( float f )
{
	sprintf( s_work, "%f", f );
	StreamWork();
	return *this;
...
UStringStreamIn &UStringStreamIn::operator <<( double d )
{
	sprintf( s_work, "%f", d );
	StreamWork();
	return *this;
...
UStringStreamIn &UStringStreamIn::operator <<( char c )
{
	s_work[0] = c;
	s_work[1] = 0;
	StreamWork();
