...
void UMat4::MultiplyMatrix( const UMat4 &in1, const UMat4 &in2 )
{
	int i;
	int i1;
...
void UMat4::MultiplyThis( const UMat4 &in )
{
	float tmp[ 4 ][ 4 ];
	int   i;
	int   i1;
...
float UMat4::MultiplyVector( const UVector3 &in, UVector3 &out ) const
{
	int   i;
	float w_inv;
	float w;
...
UMat4 UMat4::Ortho( float left, float right, float bottom, float top, float nearz, float farz )
{
	UMat4 that;
...
UMat4 UMat4::Frustum( float left, float right, float bottom, float top, float nearz, float farz )
{
	UMat4 that;
...
UMat4 UMat4::Translate( float x, float y, float z )
{
	UMat4 that;
...
void UMat3::ProjectVector( const UVector3 &in, UVector3 &out ) const
{
	out.x = in DOT vecs[ 0 ];
	out.y = in DOT vecs[ 1 ];
	out.z = in DOT vecs[ 2 ];
...
void UMat3::UnprojectVector( const UVector3 &in, UVector3 &out ) const
{
...
void UMat3::operator =( const UEuler &e )
{
	e.AngleVectors( &vecs[ 0 ], &vecs[ 1 ], &vecs[ 2 ] );
	vecs[ 1 ] *= -1.f;
}

void UMat3::Transpose( void )
{
	float f;
	int   i;
	int   i1;
...
void UMat3::MultiplyMatrix( const UMat3 &in1, const UMat3 &in2 )
{
	int i;
	int i1;
...
void UMat3::MultiplyThis( const UMat3 &in )
{
	UVector3 tmp[ 3 ];
	int      i;
	int      i1;
