...
static Entity *FindHeadTarget( Vector origin, Vector forward, float fov, float maxdist )
{
	float    dist, dot;
	float    fovdot = cos( fov * 0.5 * M_PI / 180.0 );
	Entity   *ent;
...
Entity *Player::FindEnemyInFOVFromTagWithOffset( float fov, float maxdist, str tagname, Vector offset )
{
	vec3_t        mat[3];
...
// ====================
Entity *Player::FindEnemyInFOV( float fov, float maxdist )
{
	vec3_t mat[3];
...
// ====================
void Player::AdjustAnglesForAttack( void )
{
	/*
	Vector dir;
	Vector newAngles;
...
// useWeapon
// ====================
void Player::useWeapon( const char *weaponname, weaponhand_t hand )
{
	Weapon *weapon;
...
void Player::useWeapon( Weapon *weapon, weaponhand_t hand )
{
	Weapon *activeWeapon;
...
// SwordAttackEvent
// ====================
void Player::SwordAttackEvent( Event *ev )
{
	Weapon       *weapon;
	weaponhand_t hand;
