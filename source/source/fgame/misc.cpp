...
bool UseAnim::GetInformation( Entity *activator, Vector *org, Vector *angles, str *animation, int *loopcount, str *state, str *camera )
{
	Entity *dest;
...
void SupplyWater::MaxWater( Event *ev )
{
	maxwater = ev->GetInteger( 1 );
}

void SupplyWater::ChargeOff( Event *ev )
{
	LoopSound( "snd_edenwater_ambient" );
}

void SupplyWater::Activate( Event *ev )
{
	Entity *ent;
