...
CLASS_DECLARATION( Sentient, Actor, "monster_generic" )
{
	...
	{ &EV_Actor_Friend, &Actor::FriendEvent },
	{ &EV_Actor_Civilian, &Actor::CivilianEvent },
	{ &EV_Actor_Enemy, &Actor::EnemyEvent },
	{ &EV_Actor_Monster, &Actor::MonsterEvent },
	{ &EV_Actor_Animal, &Actor::AnimalEvent },
	{ &EV_Actor_Inanimate, &Actor::InanimateEvent },
	...
	{ &EV_Actor_FinishedBehavior, &Actor::FinishedBehavior },
	...
	{ &EV_Actor_AddHealth, &Actor::AddHealth },
	...
	{ &EV_Actor_WatchOffset, &Actor::SetWatchOffset },

	{ &EV_ScriptThread_Goto, &Actor::GotoEvent },
	...
	{ &EV_Actor_AIDeaf, &Actor::Deaf },
	{ &EV_Actor_Deaf, &Actor::PermanentDeaf },
	{ &EV_Actor_Blind, &Actor::PermanentBlind },
	{ &EV_Actor_AIDumb, &Actor::Dumb },
	...
	{ &EV_ActorOnlyShootable, &Actor::OnlyShootable },
	...
	{ &EV_Actor_GetNearestEnemy, &Actor::GetNearestEnemy },
	{ &EV_Actor_GetRandomEnemy, &Actor::GetRandomEnemy },
	...
	{ &EV_Actor_AttackMode, &Actor::AttackModeEvent },
	...
	{ &EV_Actor_SetAttackableByActors, &Actor::SetAttackableByActors },
	{ &EV_Actor_SetAttackActors, &Actor::SetAttackActors },

	{ &EV_Actor_SetTargetable, &Actor::SetTargetable },

	{ &EV_Actor_ChangeType, &Actor::ChangeType },

	{ &EV_Actor_MinimumMeleeHeight, &Actor::MinimumMeleeHeight },
	{ &EV_Actor_SetDamageAngles, &Actor::SetDamageAngles },

	{ &EV_Actor_Immortal, &Actor::SetImmortal },

	{ &EV_Actor_HeadTwitch, &Actor::HeadTwitch },
	{ &EV_Actor_HeadTwitchEveryFrame, &Actor::HeadTwitchEveryFrame },
	...
	{ &EV_Actor_TouchTriggers, &Actor::TouchTriggers },
	{ &EV_Actor_IgnoreWater, &Actor::IgnoreWater },
	{ &EV_Actor_NeverIgnoreSounds, &Actor::NeverIgnoreSounds },

	{ &EV_Actor_SimplePathfinding, &Actor::SimplePathfinding },

	{ &EV_Actor_NoPainSounds, &Actor::NoPainSounds },

	{ &EV_Actor_IncrementNumSpawns, &Actor::IncrementNumSpawns },
	{ &EV_Actor_DecrementNumSpawns, &Actor::DecrementNumSpawns },

	{ &EV_Actor_SetWaterLevel, &Actor::SetWaterLevel },
	...
	{ &EV_Actor_AlwaysGiveWater, &Actor::AlwaysGiveWater },
...
qboolean Actor::WithinVisionDistance( Entity *ent )
{
	float distance;
...
inline qboolean Actor::InFOV( Vector pos, float check_fov, float check_fovdot )
{
	Vector delta;
	float  dot;
...
qboolean Actor::EnemyCanSeeMeFrom( Vector pos )
{
	Vector d;
	Vector p1;
	Vector p2;
...
qboolean Actor::CanReallySee( Entity *ent )
{
...
void Actor::SaveAttack( Vector orig, Vector dir )
{
	Vector   attack_mins;
	Vector   attack_maxs;
	Vector   end;
...
qboolean Actor::ShouldAttackEntity( Entity *ent )
{
...
void Actor::MinimumMeleeHeight( Event *ev )
{
...
void Actor::GetNearestEnemy( Event *ev )
{
...
void Actor::GetRandomEnemy( Event *ev )
{
	float     max_distance;
	float     max_distance2;
	Entity    *ent_in_range;
...
qboolean Actor::EntityInRange( Entity *ent, float range, float min_height, float max_height )
{
	float  r;
	Vector delta;
	float  height_diff;
...
void Actor::SetupThread( str filename, str label )
{
...
void Actor::SetThread( str filename, str label )
{
	str start;
...
PathNode *Actor::NearestNodeInPVS( Vector pos )
{
	Vector   delta;
	PathNode *node;
	PathNode *bestnode;
...
void Actor::RepostEvent( Event *ev, Event &event_type )
{
	Event *event;
	int   i;
	str   token;
...
qboolean Actor::IgnoreSounds( void )
{
...
void Actor::NeverIgnoreSounds( Event *ev )
{
...
qboolean Actor::CanMoveTo( Vector pos )
{
	Vector  min;
	trace_t trace;
	Vector  start;
...
qboolean Actor::CanWalkTo( Vector pos, int entnum )
{
	trace_t trace;
	Vector  dir;
	float   length;
...
void Actor::setAngles( Vector ang )
{
...
qboolean Actor::Push( Vector dir )
{
	Vector        oldorg;
	Vector        neworg;
	static Vector step( 0, 0, STEPSIZE );
...
void Actor::Deaf( Event *ev )
{
	stimuli &= ~STIMULI_SOUND;
}

void Actor::PermanentDeaf( Event *ev )
{
	permanent_stimuli &= ~STIMULI_SOUND;
}

void Actor::PermanentBlind( Event *ev )
{
	permanent_stimuli &= ~STIMULI_SIGHT;
}

qboolean Actor::IsBlind( void )
{
...
qboolean Actor::checkenemyshield( Conditional &condition )
{
	if( !currentEnemy )
		return false;
...
Condition<Actor> Actor::Conditions[] =
{
	...
	{ "ENEMY_HAS_SHIELD", &Actor::checkenemyshield },
...
{
	{ &EV_SpinningPlant_GetClip, &SpinningPlant::GetClip },
	{ NULL, NULL }
};
...
void SpinningPlant::GetClip( Event *ev )
{
	Entity *ent;
...
void Actor::PlayDialog( Sentient *user, float volume, float min_dist, const char *dialog_name, const char *state_name )
{
	DialogNode_t   *dialog_node;
	int            good_dialog;
	int            i;
...
void Actor::HeadTwitch( Event *ev )
{
	qboolean end = false;
...
void Actor::HeadTwitchEveryFrame( Event *ev )
{
	Vector real_head_angles;
	float  pitch_change;
	float  roll_change;
...
void Actor::AlwaysGiveWater( Event *ev )
{
...
void Actor::SetWaterLevel( Event *ev )
{
	water_level = ev->GetFloat( 1 );
}

void Actor::IncrementNumSpawns( Event *ev )
{
	num_of_spawns++;
}

void Actor::DecrementNumSpawns( Event *ev )
{
	num_of_spawns--;
...
void Actor::SpawnItem( str spawn_item_name )
{
	SpawnArgs args;
	Entity    *ent;
	Item      *item;
...
void Actor::SetWatchOffset( Event *ev )
{
...
bool Actor::IsEntityAlive( Entity *ent )
{
...
void Actor::OnlyShootable( Event *ev )
{
...
qboolean Actor::GetClosestTag( str tag_name, int number_of_tags, Vector target, Vector *orig )
{
	str      temp_tag_name;
	Vector   temp_orig;
	Vector   diff;
...
void Actor::SpawnActor( str model_name, Vector orig, Vector ang, int how_many, qboolean attack, float width, float height )
{
	Actor   *new_actor;
	int     current_actor;
	trace_t trace;
