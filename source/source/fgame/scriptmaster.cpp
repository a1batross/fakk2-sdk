...
void ScriptMaster::KillThread( str name )
{
	int          i;
	int          num;
	int          len;
...
void ScriptMaster::SetCurrentThread( ScriptThread *thread )
{
	currentThread = thread;
}

ScriptThread *ScriptMaster::CreateThread( scripttype_t type )
{
	ScriptThread *thread;
...
ScriptThread *ScriptMaster::CreateThread( GameScript *scr, const char *label, scripttype_t type )
{
	ScriptThread *thread;
	int          threadnum;
...
ScriptThread *ScriptMaster::CreateThread( const char *name, scripttype_t type, const char *label )
{
	GameScript   *scr;
	ScriptThread *thread;
...
ScriptThread *ScriptMaster::GetThread( int num )
{
	int          i;
	int          n;
	ScriptThread *thread;
...
qboolean ScriptMaster::labelExists( GameScript *scr, const char *name )
{
	return scr->labelExists( name );
}

qboolean ScriptMaster::labelExists( const char *filename, const char *name )
{
	GameScript *scr;
...
qboolean ScriptMaster::Goto( GameScript *scr, const char *name )
{
	return scr->Goto( name );
}

bool ScriptMaster::isVarGroup( const char *name )
{
	const char *v;
	char       vargroup[ 20 ];
	int        len;
...
ScriptVariableList *ScriptMaster::GetVarGroup( const char *name )
{
...
ScriptVariable *ScriptMaster::GetExistingVariable( const char *name )
{
	ScriptVariableList *vars;
	const char         *v;
...
ScriptVariable *ScriptMaster::GetVariable( const char *name )
{
	ScriptVariable     *var;
	ScriptVariableList *vars;
	const char         *v;
...
{
	{ &EV_ScriptThread_Execute, &ScriptThread::Execute },
	{ &EV_MoveDone, &ScriptThread::ObjectMoveDone },
	{ &EV_ScriptThread_Callback, &ScriptThread::ScriptCallback },
	{ &EV_ScriptThread_ThreadCallback, &ScriptThread::ThreadCallback },
	{ &EV_ScriptThread_VariableCallback, &ScriptThread::VariableCallback },
	{ &EV_ScriptThread_DeathCallback, &ScriptThread::DeathCallback },
	{ &EV_ScriptThread_CreateThread, &ScriptThread::CreateThread },
	{ &EV_ScriptThread_TerminateThread, &ScriptThread::TerminateThread },
	{ &EV_ScriptThread_ControlObject, &ScriptThread::ControlObject },
	...
	{ &EV_ScriptThread_WaitFor, &ScriptThread::EventWaitFor },
	{ &EV_ScriptThread_WaitForThread, &ScriptThread::EventWaitForThread },
	{ &EV_ScriptThread_WaitForVariable, &ScriptThread::EventWaitForVariable },
	{ &EV_ScriptThread_WaitForDeath, &ScriptThread::EventWaitForDeath },
	{ &EV_ScriptThread_WaitForSound, &ScriptThread::EventWaitForSound },
	{ &EV_ScriptThread_WaitForDialog, &ScriptThread::EventWaitForDialog },
	{ &EV_ScriptThread_WaitForPlayer, &ScriptThread::EventWaitForPlayer },
	{ &EV_ScriptThread_End, &ScriptThread::EventEnd },
	{ &EV_ScriptThread_Print, &ScriptThread::Print },
	{ &EV_ScriptThread_PrintInt, &ScriptThread::PrintInt },
	{ &EV_ScriptThread_PrintFloat, &ScriptThread::PrintFloat },
	{ &EV_ScriptThread_PrintVector, &ScriptThread::PrintVector },
	{ &EV_ScriptThread_NewLine, &ScriptThread::NewLine },
	{ &EV_ScriptThread_Assert, &ScriptThread::Assert },
	{ &EV_ScriptThread_Break, &ScriptThread::Break },
	{ &EV_ScriptThread_Clear, &ScriptThread::Clear },
	{ &EV_ScriptThread_Trigger, &ScriptThread::TriggerEvent },
	{ &EV_ScriptThread_ServerOnly, &ScriptThread::ServerEvent },
	{ &EV_ScriptThread_ClientOnly, &ScriptThread::ClientEvent },
	...
	{ &EV_ScriptThread_RegisterAlias, &ScriptThread::RegisterAlias },
	{ &EV_ScriptThread_RegisterAliasAndCache, &ScriptThread::RegisterAliasAndCache },
	...
	{ &EV_ScriptThread_SetAllAIOff, &ScriptThread::SetAllAIOff },
	{ &EV_ScriptThread_SetAllAIOn, &ScriptThread::SetAllAIOn },
	{ &EV_ScriptThread_SetSkipThread, &ScriptThread::SetSkipThread },
	...
	{ &EV_ScriptThread_FakePlayer, &ScriptThread::FakePlayer },
	{ &EV_ScriptThread_RemoveFakePlayer, &ScriptThread::RemoveFakePlayer },
	{ &EV_ScriptThread_SetDialogScript, &ScriptThread::SetDialogScript },
	{ &EV_ScriptThread_SetLightStyle, &ScriptThread::SetLightStyle },
	{ &EV_ScriptThread_KillThreadEvent, &ScriptThread::KillThreadEvent },
	{ &EV_ScriptThread_SetThreadNameEvent, &ScriptThread::SetThreadName },
	...
	{ &EV_AI_RecalcPaths, &ScriptThread::PassToPathmanager },
	{ &EV_AI_CalcPath, &ScriptThread::PassToPathmanager },
	{ &EV_AI_DisconnectPath, &ScriptThread::PassToPathmanager },
	{ NULL, NULL }
};
...
void ScriptThread::ClearWaitFor( void )
{
	waitUntil = 0;
	waitingFor = "";
	waitingNumObjects = 0;
...
void ScriptThread::SetType( scripttype_t newtype )
{
	type = newtype;
}

scripttype_t ScriptThread::GetType( void )
{
	return type;
}

void ScriptThread::SetThreadNum( int num )
{
	threadNum = num;
}

int ScriptThread::ThreadNum( void )
{
	return threadNum;
}

const char *ScriptThread::ThreadName( void )
{
	return threadName.c_str();
}

void ScriptThread::SetThreadName( Event *ev )
{
	threadName = ev->GetString( 1 );
}

int ScriptThread::CurrentLine( void )
{
	return linenumber;
}

const char *ScriptThread::Filename( void )
{
	return script.Filename();
}

ScriptThread *ScriptThread::WaitingOnThread( void )
{
	return waitingForThread;
}

const char *ScriptThread::WaitingOnVariable( void )
{
	return waitingForVariable.c_str();
}

const char *ScriptThread::WaitingOnDeath( void )
{
	return waitingForDeath.c_str();
}

qboolean ScriptThread::WaitingOnPlayer( void )
{
	return waitingForPlayer;
}

ScriptVariableList *ScriptThread::Vars( void )
{
	return &localVars;
}

qboolean ScriptThread::Setup( int num, GameScript *scr, const char *label )
{
	threadNum = num;
...
qboolean ScriptThread::SetScript( const char *name )
{
	GameScript *scr;
...
qboolean ScriptThread::Goto( const char *name )
{
	qboolean result;
...
qboolean ScriptThread::labelExists( const char *name )
{
	return ScriptLib.labelExists( &script, name );
}

void ScriptThread::Mark( ThreadMarker *mark )
{
	assert( mark );
...
void ScriptThread::Restore( ThreadMarker *mark )
{
	assert( mark );
...
void ScriptThread::SendCommandToSlaves( const char *name, Event *ev )
{
	Event      *sendevent;
	Entity     *ent;
	TargetList *tlist;
...
void ScriptThread::ProcessCommand( int argc, const char **argv )
{
	ScriptVariableList *vars;
	ScriptVariable     *var;
	str    command;
...
void ScriptThread::Start( float delay )
{
	CancelEventsOfType( EV_ScriptThread_Execute );
	PostEvent( EV_ScriptThread_Execute, delay );
}

void ScriptThread::StartImmediately( void )
{
	CancelEventsOfType( EV_ScriptThread_Execute );
...
void ScriptThread::ScriptError( const char *fmt, ... )
{
	va_list argptr;
	char    text[ 1024 ];
...
qboolean ScriptThread::WaitingFor( Entity *obj )
{
	assert( obj );
...
void ScriptThread::ObjectMoveDone( Event *ev )
{
	Entity *obj;
...
void ScriptThread::EventWaitForDialog( Event *ev )
{
	float  delay;
	Actor  *act;
	Entity *ent;
...
void ScriptThread::ScriptCallback( Event *ev )
{
	char   name[ MAXTOKEN ];
	Entity *other;
	Entity *slave;
...
void ScriptThread::ThreadCallback( Event *ev )
{
	ScriptThread *thread;
...
void ScriptThread::VariableCallback( Event *ev )
{
	ClearWaitFor();
	// start right away
	Start( 0 );
...
void ScriptThread::DeathCallback( Event *ev )
{
	if( threadDying )
	{
		return;
...
void ScriptThread::FakePlayer( Event *ev )
{
	qboolean holster;
	Player   *player;
...
void ScriptThread::RemoveFakePlayer( Event *ev )
{
	Player *player;
...
void ScriptThread::KillThreadEvent( Event *ev )
{
