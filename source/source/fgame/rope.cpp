...
{
	{ &EV_Touch, &Rope::Touch },
	{ &EV_Rope_Setup, &Rope::Setup },
	{ &EV_Rope_Think, &Rope::RopeThink },
	{ NULL, NULL }
};
...
void Rope::SetShader( str name )
{
	shaderName = name;
	edict->s.surfaces[ 0 ] = gi.imageindex( shaderName );
}

void Rope::SetShader( Event *ev )
{
	SetShader( ev->GetString( 1 ));
}

void Rope::Update( Vector midpos )
{
...
void Rope::Grab( Player *player, float offset )
{
	if( player )
	{
		float  dot;
...
void Rope::Release( Player *grabber )
{
...
bool Rope::CanClimb( float distance )
{
...
void Rope::Climb( float distance )
{
	Vector start, end;
	float  newriderpos;
...
Vector Rope::CalculatePosition( float position )
{
	Vector dir, endpos;
...
Vector Rope::CalculateRiderPosition( Vector position )
{
	Vector endpos;
...
bool Rope::CanMove( Vector start, Vector location )
{
	trace_t trace;
...
void Rope::Move( Vector location )
{
	if( rider )
	{
		Vector pos;
...
{
	{ &EV_RopePiece_Setup, &RopePiece::Setup },
	{ &EV_Activate, &RopePiece::PieceTriggered },
	{ &EV_RopePiece_WiggleTime, &RopePiece::SetWiggleTime },
	{ &EV_RopePiece_WiggleMove, &RopePiece::SetWiggleMove },
	{ &EV_RopePiece_Damage, &RopePiece::SetDamage },
	{ &EV_RopePiece_Target2, &RopePiece::Target2 },
	{ &EV_RopePiece_AttachModel, &RopePiece::AttachModelToRope },
	{ NULL, NULL }
};
...
void RopePiece::AttachModelToRope( Event *ev )
{
...
void RopePiece::SetWiggleTime( Event *ev )
{
	wiggletime = ev->GetFloat( 1 );
}

void RopePiece::SetWiggleMove( Event *ev )
{
	wigglemove = ev->GetFloat( 1 );
}

void RopePiece::Target2( Event *ev )
{
	target2 = ev->GetString( 1 );
}

void RopePiece::setOrigin( Vector org )
{
...
{
	{ &EV_RopeBase_Setup, &RopeBase::Setup },
	{ &EV_RopeBase_Stiffness, &RopeBase::SetStiffness },
	{ &EV_RopeBase_Strength, &RopeBase::SetStrength },
	{ &EV_RopeBase_PieceLength, &RopeBase::SetPieceLength },
	{ &EV_RopeBase_RopeDampener, &RopeBase::SetRopeDampener },
	{ &EV_RopeBase_RopeShader, &RopeBase::SetShader },
	{ &EV_Activate, &RopeBase::Activate },
	{ NULL, NULL }
};
...
void RopeBase::SetShader( str name )
{
	shaderName = name;
	edict->s.surfaces[ 1 ] = gi.imageindex( shaderName );
}

void RopeBase::SetShader( Event *ev )
{
	SetShader( ev->GetString( 1 ));
}

void RopeBase::SetStiffness( Event *ev )
{
...
void RopeBase::SetStrength( Event *ev )
{
	strength = ev->GetFloat( 1 );
}

void RopeBase::SetPieceLength( Event *ev )
{
	piecelength = ev->GetFloat( 1 );
}

void RopeBase::SetRopeDampener( Event *ev )
{
