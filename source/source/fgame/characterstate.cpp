...
Conditional::Conditional( Condition<Class> &cond ) : condition( cond )
{
	result = false;
...
State::State( const char *statename, Script &script, StateMap &map ) : statemap( map )
{
	str cmd;
...
StateMap *GetStatemap( str filename, Condition<Class> *conditions, Container<Conditional *> *conditionals, qboolean reload, qboolean cache_only )
{
	int               i;
	int               j;
