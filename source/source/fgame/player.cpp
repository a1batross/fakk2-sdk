...
CLASS_DECLARATION( Sentient, Player, "player" )
{
	...
	{ &EV_Player_SwordAttack, &Player::SwordAttackEvent },
	{ &EV_Player_SwipeOn, &Player::SwipeOn },
	{ &EV_Player_SwipeOff, &Player::SwipeOff },
	{ &EV_Player_SetHeadTarget, &Player::SetHeadTarget },
	{ &EV_Player_ChangeOutfit, &Player::ChangeOutfit },
	{ &EV_Player_ListInventory, &Player::ListInventoryEvent },
	{ &EV_Player_PickupShgliek, &Player::PickupShgliek },
	{ &EV_Player_ThrowShgliek, &Player::ThrowShgliek },
	{ &EV_Player_ClearLeftArmTarget, &Player::ClearLeftArmTarget },
	{ &EV_Player_ClearRightArmTarget, &Player::ClearRightArmTarget },
	...
	{ &EV_Player_StepUp, &Player::StepUp },
	...
	{ &EV_Player_SetWaterPower, &Player::SetWaterPower },
...
qboolean Player::checkcanmoveleft( Conditional &condition )
{
...
qboolean Player::checkcanmoveright( Conditional &condition )
{
	if( condition.numParms())
	{
...
qboolean Player::checkcanmovebackward( Conditional &condition )
{
	if( condition.numParms())
	{
...
qboolean Player::checkcanmoveforward( Conditional &condition )
{
	if( condition.numParms())
	{
...
qboolean Player::checkcanwallhug( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkhangatwall( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkcanhang( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkcanhangleft( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkcanhangright( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checktouchrope( Conditional &condition )
{
	return( rope_touch != NULL );
}

qboolean Player::checkonrope( Conditional &condition )
{
	return( rope_grabbed != NULL );
}

qboolean Player::checkcanclimbrope( Conditional &condition )
{
	if( rope_grabbed )
	{
...
qboolean Player::checkatshgliek( Conditional &condition )
{
	Vector  start;
	Vector  end;
	trace_t trace;
...
qboolean Player::checkhaveshgliek( Conditional &condition )
{
	return have_shgliek;
}

qboolean Player::checkshgliekdead( Conditional &condition )
{
	int    i;
	Entity *child;
	Actor  *act;
...
qboolean Player::checkcangrabledge( Conditional &condition )
{
...
qboolean Player::checkledgeheight( Conditional &condition )
{
	float deltaleft;
	float deltaright;
...
qboolean Player::checkcangrabpipe( Conditional &condition )
{
	return horizontalpipe != NULL;
}

qboolean Player::checkcangrabmonkeybar( Conditional &condition )
{
	float old_hang_pullup_distance;
	float hang_pullup_distance;
	float deltaleft;
...
qboolean Player::checkcanpullup( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkmuzzleclear( Conditional &condition )
{
	weaponhand_t hand;
...
qboolean Player::checkweaponreadytofire_nosound( Conditional &condition )
{
	firemode_t   mode = FIRE_PRIMARY;
	str          weaponName = "None";
	weaponhand_t hand;
...
qboolean Player::checkladder( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkcanclimbladder( Conditional &condition )
{
	trace_t trace;
	Vector  newmins;
	Vector  vec;
...
qboolean Player::checkfeetatladder( Conditional &condition )
{
	trace_t trace;
...
qboolean Player::checkspecialmove( Conditional &condition )
{
	weaponhand_t hand;
...
qboolean Player::checkwaterlevel( Conditional &condition )
{
	return( water_power >= atof( condition.getParm( 1 )));
}

qboolean Player::checkpickupenemy( Conditional &condition )
{
	return pickup_enemy;
}
...
qboolean Player::checkcoolitem( Conditional &condition )
{
	return( cool_item != NULL );
}

qboolean Player::checkarmorpiece( Conditional &condition )
{
...
Condition<Player> Player::Conditions[] =
{
	...
	{ "HANG_ATWALL", &Player::checkhangatwall },
	{ "TOUCHED_ROPE", &Player::checktouchrope },
	{ "ONROPE", &Player::checkonrope },
	{ "CAN_CLIMB_ROPE", &Player::checkcanclimbrope },
	...
	{ "CAN_HANG_LEFT", &Player::checkcanhangleft },
	{ "CAN_HANG_RIGHT", &Player::checkcanhangright },
	{ "CAN_HANG", &Player::checkcanhang },
	{ "CAN_FALL", &Player::checkcanfall },
	{ "AT_DOOR", &Player::checkatdoor },
	{ "AT_SHGLIEK", &Player::checkatshgliek },
	{ "HAVE_SHGLIEK", &Player::checkhaveshgliek },
	{ "SHGLIEK_DEAD", &Player::checkshgliekdead },
	{ "FALLING", &Player::checkfalling },
	{ "HARD_IMPACT", &Player::checkhardimpact },
	{ "CAN_GRAB_LEDGE", &Player::checkcangrabledge },
	{ "LEDGE_HEIGHT", &Player::checkledgeheight },
	{ "CAN_GRAB_PIPE", &Player::checkcangrabpipe },
	{ "CAN_GRAB_MONKEYBARS", &Player::checkcangrabmonkeybar },
	{ "CAN_PULLUP", &Player::checkcanpullup },
	...
	{ "CAN_CLIMB_LADDER", &Player::checkcanclimbladder },
	...
	{ "WATER_LEVEL", &Player::checkwaterlevel },
	{ "PICKUP_ENEMY", &Player::checkpickupenemy },
	{ "SOLID_FORWARD", &Player::checksolidforward },
	{ "GOT_COOL_ITEM", &Player::checkcoolitem },
	...
	{ "HAS_ARMORPIECE", &Player::checkarmorpiece },
	{ "SPECIALMOVE", &Player::checkspecialmove },
...
movecontrolfunc_t Player::MoveStartFuncs[] =
{
	...
	&Player::StartHang,         // MOVECONTROL_HANGING,    // move based on animation, with full collision testing, hanging
	NULL,              // MOVECONTROL_WALLHUG,    // move based on animation, with full collision testing, hanging
	&Player::StartMonkeyBars,   // MOVECONTROL_MONKEYBARS, // move based on animation, with full collision testing, monkey bars
	&Player::StartPipeCrawl,    // MOVECONTROL_PIPECRAWL,  // move based on animation, with full collision testing, crawling on pipe
	&Player::StartPipeCrawl,    // MOVECONTROL_PIPEHANG,   // move based on animation, with full collision testing, hanging from pipe
	&Player::StartStepUp,       // MOVECONTROL_STEPUP,
	&Player::RopeGrab,          // MOVECONTROL_ROPE_GRAB
	&Player::RopeRelease,       // MOVECONTROL_ROPE_RELEASE
	NULL,              // MOVECONTROL_ROPE_MOVE
	&Player::StartPickup,       // MOVECONTROL_PICKUPENEMY
	&Player::StartPush,         // MOVECONTROL_PUSH
	&Player::StartClimbWall,    // MOVECONTROL_CLIMBWALL
...
void Player::InitWaterPower( void )
{
	water_power = 50;
	max_water_power = 100;
...
void Player::Obituary( Entity *attacker, Entity *inflictor, int meansofdeath )
{
	if( !deathmatch->integer )
	{
		return;
...
void Player::PickupShgliek( Event *ev )
{
	Vector  start;
	Vector  end;
	trace_t trace;
...
void Player::ThrowShgliek( Event *ev )
{
	int     i;
	int     num;
	Entity  *child;
...
void Player::StartPipeCrawl( void )
{
	trace_t trace;
	float   ang;
	float   dist;
...
void Player::StartMonkeyBars( void )
{
	trace_t trace;
	Vector  oldang( v_angle );
...
void Player::SetPlayerView( Camera *camera, Vector position, float cameraoffset, Vector ang, Vector vel, float camerablend[ 4 ], float camerafov )
{
...
==================
*/
void Player::SwingAngles( float destination, float swingTolerance, float clampTolerance, float speed, float *angle, qboolean *swinging )
{
	float swing;
	float move;
	float scale;
...
void Player::SetHeadTarget( Event *ev )
{
...
qboolean Player::GetTagPositionAndOrientation( str tagname, orientation_t *new_or )
{
	int tagnum;
...
Vector Player::CalcArmTracking( int controller_tag, str weapon_tagname, Vector baseAngles, Entity *target, Weapon *weapon )
{
...
void Player::StatCount( int index, int count )
{
	// If stat is going up
...
void Player::SetAnim( const char *anim, bodypart_t part )
{
	assert( anim );
...
void Player::UpdateSword( Sword *sword )
{
...
void Player::UpdateWeapons( void )
{
...
float Player::GetWaterPower( void )
{
	return water_power;
}

void Player::SetWaterPower( float w )
{
	float          diff;
...
void Player::ChangeOutfit( Event *ev )
{
...
void Player::SetOutfit( int stage )
{
...
int Player::GetOutfit( void )
{
	return outfit_level;
}

void Player::ClearLeftArmTarget( Event *ev )
{
	left_arm_target = NULL;
}

void Player::ClearRightArmTarget( Event *ev )
{
	right_arm_target = NULL;
}
...
void Player::GrabRope( Entity *ent )
{
	rope_grabbed = (Rope *)ent;
}

void Player::TouchRope( Entity *ent )
{
	rope_touch = (Rope *)ent;
}

qboolean Player::OnRope( void )
{
	return( rope_grabbed != NULL );
}

void Player::SetWaterPower( Event *ev )
{
