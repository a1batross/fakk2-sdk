...
{
	{ &EV_InventoryItem_Use, &WaterInventoryItem::Use },
	{ NULL, NULL }
};


void WaterInventoryItem::Use( Event *ev )
{
	Event  *event;
	Entity *other;
	str    sound_name;
...
{
	{ &EV_Trigger_Effect, &HealthPlant::Touch },
	{ &EV_HealthPlant_SetFallVelocity, &HealthPlant::SetFallVelocity },
	{ NULL, NULL }
};
...
void HealthPlant::Touch( Event *ev )
{
	Entity *other;
	Event  *e;
...
void HealthPlant::SetFallVelocity( Event *ev )
{
	fall_velocity = ev->GetVector( 1 );
}

void HealthPlant::Hit( Vector velocity, qboolean fall_straight_down )
{
	Health *health;
	Vector orig;
