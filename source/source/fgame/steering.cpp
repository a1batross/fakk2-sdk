...
void ObstacleAvoidance::SetTargetEnt( Entity *ent )
{
	target_ent = ent;
}
...
void FollowPath::DrawForces( void )
{
	seek.DrawForces();
}
...
void Chase::AllowFail( qboolean fail )
{
	allow_fail = fail;
}

void Chase::UseNearestNode( qboolean use_nearest )
{
...
void Jump::SetGoal( const char *node_name )
{
	PathNode *movegoal;
...
void Jump::SetSpeed( float jump_speed )
{
	speed = jump_speed;
}
