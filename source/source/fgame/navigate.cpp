...
void PathNode::setAngles( Vector ang )
{
	angles = ang;
}
...
void PathNode::Setup( Vector pos )
{
...
qboolean PathNode::TestMove( Entity *ent, Vector start, Vector end, Vector &min, Vector &max, qboolean allowdoors, qboolean fulltest )
{
...
qboolean PathNode::CheckMove( Entity *ent, Vector pos, Vector &min, Vector &max, qboolean allowdoors, qboolean fulltest )
{
...
Door *PathNode::CheckDoor( Vector pos )
{
	trace_t trace;
	Entity  *ent;
...
qboolean PathNode::CheckMove( Vector pos, Vector min, Vector max )
{
	return true;
}

qboolean PathNode::CheckPath( PathNode *node, Vector min, Vector max, qboolean fulltest )
{
	Vector   delta;
	qboolean allowdoors;
	qboolean result;
...
qboolean PathNode::LadderTo( PathNode *node, byte maxheight[ NUM_WIDTH_VALUES ] )
{
	int      i;
	int      j;
	int      m;
...
void PathNode::ConnectTo( PathNode *node, byte maxheight[ NUM_WIDTH_VALUES ], float cost, Door *door )
{
	int i;
...
void PathNode::ConnectTo( PathNode *node, byte maxheight[ NUM_WIDTH_VALUES ] )
{
	Vector delta;
	Door   *door;
...
void PathNode::Disconnect( PathNode *node )
{
	int i;
...
{
	{ &EV_AI_SavePaths, &PathSearch::SavePathsEvent },
	{ &EV_AI_LoadNodes, &PathSearch::LoadNodes },
	{ &EV_AI_SaveNodes, &PathSearch::SaveNodes },
	{ &EV_AI_ClearNodes, &PathSearch::ClearNodes },
	{ &EV_AI_SetNodeFlags, &PathSearch::SetNodeFlagsEvent },
	{ &EV_AI_RecalcPaths, &PathSearch::RecalcPathsEvent },
	{ &EV_AI_CalcPath, &PathSearch::CalcPathEvent },
	{ &EV_AI_DisconnectPath, &PathSearch::DisconnectPathEvent },
...
PathNode *PathSearch::NearestNode( Vector pos, Entity *ent, qboolean usebbox )
{
	Vector   delta;
	PathNode *node;
	PathNode *bestnode;
...
void PathSearch::Teleport( Entity *teleportee, Vector from, Vector to )
{
	PathNode *node1;
	PathNode *node2;
	byte     maxheight[ NUM_WIDTH_VALUES ];
...
int PathSearch::NumNodes( void )
{
	return numNodes;
}

int PathSearch::NumLoadNodes( void )
{
	return numLoadNodes;
}

void PathSearch::Checksum( Vector &checksum )
{
	checksum = pathNodesChecksum;
}

void PathSearch::Archive( Archiver &arc )
{
	int i;
	int num;
...
void PathSearch::ClearNodes( Event *ev )
{
	PathNode *node;
	int      i;
	int      num;
...
void PathSearch::SetNodeFlagsEvent( Event *ev )
{
	const char *token;
	int        i, argnum;
	int        mask;
...
void PathSearch::CalcPathEvent( Event *ev )
{
	int      nodenum;
	PathNode *node;
	PathNode *node2;
...
void PathSearch::DisconnectPathEvent( Event *ev )
{
	int      nodenum;
	PathNode *node;
	PathNode *node2;
...
void PathSearch::RecalcPathsEvent( Event *ev )
{
	int      nodenum;
	PathNode *node;
...
qboolean PathSearch::ArchiveNodes( str name, qboolean save )
{
	Archiver arc;
	qboolean success;
...
void PathSearch::SaveNodes( Event *ev )
{
	str name;
...
void PathSearch::LoadNodes( Event *ev )
{
	Archiver arc;
	str      name;
	bool     rebuild;
...
void PathSearch::SavePaths( void )
{
	str   filename;
	Event *ev;
...
void PathSearch::SavePathsEvent( Event *ev )
{
	str temp;
...
void PathSearch::Init( const char *mapname )
{
	int   x;
	int   y;
	str   filename;
