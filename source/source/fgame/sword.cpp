...
void Sword::HitSpawn( Vector origin, firemode_t mode )
{
...
float Sword::GetWaterDamage( firemode_t mode )
{
	assert(( mode >= 0 ) && ( mode < MAX_FIREMODES ));
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
void Sword::SetBasicDamageEvent( Event *ev )
{
	assert(( firemodeindex >= 0 ) && ( firemodeindex < MAX_FIREMODES ));
	basicDamage[firemodeindex] = ev->GetFloat( 1 );
}

void Sword::SetBasicDamage( float amount, firemode_t mode )
{
	assert(( mode >= 0 ) && ( mode < MAX_FIREMODES ));
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
float Sword::GetBasicDamage( firemode_t mode )
{
	assert(( mode >= 0 ) && ( mode < MAX_FIREMODES ));
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
void Sword::SetWaterRequiredEvent( Event *ev )
{
	assert(( firemodeindex >= 0 ) && ( firemodeindex < MAX_FIREMODES ));
	waterRequired[firemodeindex] = ev->GetFloat( 1 );
}

void Sword::SetWaterRequired( int amount, firemode_t mode )
{
	assert(( mode >= 0 ) && ( mode < MAX_FIREMODES ));
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
int Sword::GetWaterRequired( firemode_t mode )
{
	assert(( mode >= 0 ) && ( mode < MAX_FIREMODES ));
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
void Sword::SetBasicKnockbackEvent( Event *ev )
{
	assert(( firemodeindex >= 0 ) && ( firemodeindex < MAX_FIREMODES ));
	basicKnockback[firemodeindex] = ev->GetFloat( 1 );
}

float Sword::GetBasicKnockback( firemode_t mode )
{
	assert(( mode >= 0 ) && ( mode < MAX_FIREMODES ));
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
void Sword::SetWaterKnockbackEvent( Event *ev )
{
	assert(( firemodeindex >= 0 ) && ( firemodeindex < MAX_FIREMODES ));
	waterKnockback[firemodeindex] = ev->GetFloat( 1 );
}

float Sword::GetWaterKnockback( firemode_t mode )
{
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
void Sword::SetPowerOff( firemode_t mode )
{
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
void Sword::SetPowerOn( firemode_t mode )
{
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
...
qboolean Sword::GetPowerActive( firemode_t mode )
{
	if(( mode < 0 ) || ( mode >= MAX_FIREMODES ))
	{
