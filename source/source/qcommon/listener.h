...
inline void Event::SetSourceObject( Listener *source )
{
	m_sourceobject = source;
}

inline Listener *Event::GetSourceObject( void )
{
	return m_sourceobject;
}

inline SafePtr<Listener> *Event::GetSourceObjectPointer( void )
{
	return &m_sourceobject;
}
...
inline eventsource_t Event::GetSource( void )
{
	return (eventsource_t)info.source;
}

inline int Event::GetAnimationNumber( void )
{
	return anim_number;
}

inline int Event::GetAnimationFrame( void )
{
	return anim_frame;
}

inline void Event::SetAnimationNumber( int anim )
{
	anim_number = anim;
}

inline void Event::SetAnimationFrame( int frame )
{
	anim_frame = frame;
}

inline int Event::GetLineNumber( void )
{
...
inline int Event::GetFlags( void )
{
	return info.flags;
}

inline str Event::getName( void )
{
...
inline Event& Event::printInfo( void )
{
...
inline void Event::operator=( const Event &ev )
{
...
#ifdef GAME_DLL
inline void Event::AddEntity( Entity *ent )
{
	if( !data )
	{
		data = new Container<EventVar>;
...
inline const char *Event::GetString( int pos )
{
	if( !data || ( pos < 1 ) || ( data->NumObjects() < pos ))
	{
		Error( "Index %d out of range.", pos );
...
inline int Event::GetInteger( int pos )
{
	if( !data || ( pos < 1 ) || ( data->NumObjects() < pos ))
	{
		Error( "Index %d out of range.", pos );
...
inline float Event::GetFloat( int pos )
{
	if( !data || ( pos < 1 ) || ( data->NumObjects() < pos ))
	{
		Error( "Index %d out of range.", pos );
...
inline Vector Event::GetVector( int pos )
{
	if( !data || ( pos < 1 ) || ( data->NumObjects() < pos ))
	{
		Error( "Index %d out of range.", pos );
...
inline bool Event::GetBoolean( int pos )
{
	int val;
...
inline ScriptVariable *Event::GetVariable( int pos )
{
	if( !data || ( pos < 1 ) || ( data->NumObjects() < pos ))
	{
		Error( "Index %d out of range.", pos );
